...
{
    public class TribeManager : AppDataManagerBase, IManager<Tribe, TribeSearch>
    {
        ...
        public void BuildInsertUpdateParameters(Tribe entity)
        {
            if (entity.ID > 0)
            {
                AddParameter("taxonomy_tribe_id", entity.ID == 0 ? DBNull.Value : (object)entity.ID, true);
            }
            ...
            AddParameter("tribe_name", String.IsNullOrEmpty(entity.TribeName) ? DBNull.Value : (object)entity.TribeName, true);
        ...
        public int Delete(Tribe entity)
        {
            throw new NotImplementedException();
        }
        ...
        public int Insert(Tribe entity)
        {
            Reset(CommandType.StoredProcedure);
            Validate<Tribe>(entity);
            SQL = "usp_GGTools_Taxon_Tribe_Insert";

            BuildInsertUpdateParameters(entity);

            AddParameter("@out_error_number", -1, true, System.Data.DbType.Int32, System.Data.ParameterDirection.Output);
            AddParameter("@out_taxonomy_tribe_id", -1, true, System.Data.DbType.Int32, System.Data.ParameterDirection.Output);

            RowsAffected = ExecuteNonQuery();

            entity.ID = GetParameterValue<int>("@out_taxonomy_tribe_id", -1);
        ...
        public List<Tribe> Search(TribeSearch searchEntity)
        {
            List<Tribe> results = new List<Tribe>();

            SQL = " SELECT * FROM vw_GGTools_Taxon_Tribes";
            ...
            results = GetRecords<Tribe>(SQL, parameters.ToArray());
            RowsAffected = results.Count;

            return results;
        }

        public List<Tribe> SearchFolderItems(TribeSearch searchEntity)
        {
            List<Tribe> results = new List<Tribe>();

            SQL = " SELECT " +
                " auil.app_user_item_list_id AS ListID, " +  
                " auil.list_name AS ListName, " +
	            " auil.app_user_item_folder_id AS FolderID, " +
	            " vgti.* " +
                " FROM vw_GGTools_Taxon_Infrafamilies vgti " +
                " JOIN app_user_item_list auil ON vgti.ID = auil.id_number " +
                " WHERE auil.id_type = 'just_tribe' ";
